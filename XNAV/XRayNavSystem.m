classdef XRayNavSystem < handle
    ...
    methods(Access = public)
        function stateMatrix = resolve(this, initialState, initialCov, estimatorType, reportProgress)
            ...
            if strcmp(estimatorType{1}, 'pf')
                ...
                particleSet.particles           = chol(cov, 'lower')*randn(this.initArgs.stateDimension, numParticles) + cvecrep(state, numParticles);
                particleSet.weights             = ones(1, numParticles) / numParticles;
            elseif strcmp(estimatorType{1}, 'gspf')
                numParticles = 2e3;
                particleSet.particlesNum   = numParticles;
                initialParticles           = chol(cov, 'lower')*randn(this.initArgs.stateDimension, numParticles) + cvecrep(state, numParticles);
            ...
            for i = 1:num
                ...
                for j = startSample:len
                    ...
                    if reportProgress && mod((sample / simNum)*100, 5) == 0
                    ...
                    [state, cov, decompCov, param, particleSet] = this.evaluate(state, cov, decompCov, observ, estimatorType, particleSet);
    ...
    methods (Access = private)
        function [state, cov, sCov, param, particleSetEst] = evaluate(this, state, cov, sCov, observ, estimator, particleSet)
            param = [];
            particleSetEst = [];
        ...
        function decompCov = updateFilterParams(this, cov, estimatorType)
            ...
            switch estimatorType{1}
                case 'ukf'
                    ...
                    kappa = 11;     % 0 is optimal for state dimension = 2 (UKF parameter)
