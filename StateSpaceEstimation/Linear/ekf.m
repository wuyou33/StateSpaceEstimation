function [stateNew, stateCovNew, stateNoise, observNoise, internal] = ekf(state, covState, stateNoise, observNoise, observation, model, control1, control2)
    ...
    predictedObserv   = model.stateObservationFun(model, predictState, observNoise.mean, control2);
    
    if isempty(model.innovationModelFunc)
        inov = observation - predictedObserv;
    else
        inov = model.innovationModelFunc(model, observation, predictedObserv);
    end
