function [stateNew, stateSqrtCovNew, stateNoise, observNoise, internal ] = srcdkf(state, sqrtCovState, stateNoise, observNoise, observation, model, ctrl1, ctrl2)
    ...
    if stateNoise.covariance == 0
        sigmaPointSet1 = column_vector_replicate(state, numSigmaPointSet1);
        offsetSet1     = sqrtCovState;
    else
        sigmaPointSet1 = column_vector_replicate([state; stateNoise.mean], numSigmaPointSet1);
    ...
    predictState = model.transition_fun(model, sigmaPointSet1(1:stateDim, :), sigmaPointSet1(stateDim + 1 : stateDim + stateNoiseDim, :), ctrl1);
    ...
    sigmaPointSet2 = column_vector_replicate([meanPredictState; observNoise.mean], numSigmaPointSet2);
    ...
    predictedObs = model.observation_fun(model, sigmaPointSet2(1:stateDim, :), sigmaPointSet2(stateDim+1:stateDim+obsNoiseDim, :), ctrl2);
    ...
    d = w2(2,2) * ( predictedObs(:, 2:stateDim + obsNoiseDim + 1) + predictedObs(:, stateDim + obsNoiseDim + 2 : numSigmaPointSet2) - ...
        column_vector_replicate(2*predictedObs(:, 1), stateDim + obsNoiseDim));
    ...
    if isempty(model.innovation)
        ...
        inov = model.innovation( model, observation, meanPredictedObs);
    end
