function [stateNew, stateCovNew, stateNoise, observNoise, internal] = ukf(state, covState, stateNoise, observNoise, observation, model, control1, control2)
    ...
    if (procNoiseDim ~= 0)
        sigmaPoints  = column_vector_replicate([state; stateNoise.mean; observNoise.mean], numSigmaSet);
        ...
        sigmaPoints  = row_vector_replicate([state observNoise.mean], numSigmaSet);
    ...
    predictState = model.transition_fun(model, sigmaPoints(1:stateDim, :), sigmaPoints(stateDim + 1 : stateDim + procNoiseDim, :), control1);
    meanPredictState = w(1) * predictState(:, 1) + w(2)*sum(predictState(:, 2:numSigmaSet), 2);
    centrPredState = predictState - column_vector_replicate(meanPredictState, numSigmaSet);
    ...
    predictedObserv = model.observation_fun(model, predictState, sigmaPoints(stateDim+procNoiseDim+1 : stateDim+procNoiseDim+obserNoiseDim, :), control2);
    predictedObservMean = w(1)*predictedObserv(:, 1) + w(2)*sum(predictedObserv(:, 2:numSigmaSet), 2);
    centredPredicatedObserv = predictedObserv - column_vector_replicate(predictedObservMean, numSigmaSet);
    ...
    if isempty(model.innovation)
        ...
        inov = model.innovation(model, observation, predictedObservMean);
    end
