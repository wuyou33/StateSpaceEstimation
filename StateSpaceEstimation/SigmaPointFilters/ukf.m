function [newState, newCovState, processNoise, observationNoise, internalVariables] = ukf(state, covState, processNoise, observationNoise, observation, gssModel, controlProcess, controlObservation)
    ...
    for i = 1:numSigmaPoints
        predictedState(:, i) = gssModel.stateTransitionFun(gssModel, sigmaPoints(1:stateDim, i), sigmaPoints(stateDim+1 : stateDim + procNoiseDim, i), ctrlProc(:, i));
    end
    ...
    for i = 1:numSigmaPoints
        predictedObserv(:, i) = gssModel.stateObservationFun(gssModel, predictedState(:, i), sigmaPoints(stateDim+procNoiseDim+1 : stateDim+procNoiseDim+obserNoiseDim, i), ctrlObserv(:, i));
    end
