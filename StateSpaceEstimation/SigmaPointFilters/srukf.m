function [stateNew, stateSqrtCovNew, stateNoise, observNoise, internal] = srukf(state, sqrtCovState, stateNoise, observNoise, observation, model, ctrl1, ctrl2)
    ...
    if procNoiseDim ~= 0
        sigmaPoints  = column_vector_replicate([state; stateNoise.mean; observNoise.mean], numSigmaPoints);
        ...
        sigmaPoints  = column_vector_replicate([state; observNoise.mean], numSigmaPoints);
    ...
    predictState = model.transition_fun(model, sigmaPoints(1:stateDim, :), sigmaPoints(stateDim + 1 : stateDim + procNoiseDim, :), ctrl1);
    meanPredictState = w(1) * predictState(:, 1) + w(2)*sum(predictState(:, 2:numSigmaPoints), 2);
    centredPredictState = predictState - column_vector_replicate(meanPredictState, numSigmaPoints);
    ...
    predictObs = model.observation_fun(model, predictState, sigmaPoints(stateDim + procNoiseDim + 1 : stateDim + procNoiseDim+obserNoiseDim, :), ctrl2);
    predictObsMean = w(1)*predictObs(:, 1) + w(2)*sum(predictObs(:, 2:numSigmaPoints), 2);
    centredPredictObs = predictObs - column_vector_replicate(predictObsMean, numSigmaPoints);
