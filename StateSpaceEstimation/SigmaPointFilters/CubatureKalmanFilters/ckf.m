function [newState, newCovState, stateNoise, observNoise, internal ] = ckf(state, covState, stateNoise, observNoise, observation, model, control1, control2)
    ...
    offsetPrediction = svd_decomposition(covState);
    cubatureSet  = column_vector_replicate(state, numCubPointSet) + offsetPrediction*(sqrt(numCubPointSet / 2)*[eye(stateDim) -eye(stateDim)]);
    
    %% propagate cubature-points through process model
    predictedState = model.transition_fun(model, cubatureSet, column_vector_replicate(stateNoise.mean, numCubPointSet), control1);
    
    predictedStateMean = sum(predictedState, 2) / numCubPointSet;
    sqrtPredictedStateCov = (predictedState - column_vector_replicate(predictedStateMean, numCubPointSet)) / sqrt(numCubPointSet);
    ...
    offsetObs = svd_decomposition(predictedStateCov);
    cubatureSet2 = column_vector_replicate(predictedStateMean, numCubPointSet) + offsetObs*(sqrt(numCubPointSet / 2)*[eye(stateDim) -eye(stateDim)]);
    ...
    predictObs = model.observation_fun(model, cubatureSet2, column_vector_replicate(observNoise.mean, numCubPointSet), control2);
    predictObsMean = sum(predictObs, 2) / numCubPointSet;
    
    %% measurement update (correction)
    x = (cubatureSet2(1:stateDim, :) - column_vector_replicate(predictedStateMean, numCubPointSet)) / sqrt(numCubPointSet);
