function [ z ] = cartesian_product( varargin )
    % cartesian_product. Find all combinations for input set (varargin)
    %
    %   [ z ] = cartesian_product( varargin )
    %
    ...
    if any(cellfun('isempty', varargin))
        error('[ cartesian_product:EmptyInput] One of more empty inputs result in an empty output.');
    end
