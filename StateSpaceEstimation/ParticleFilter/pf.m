function [ estimate, dataSet, stateNoise, observNoise ] = pf( dataSet, stateNoise, observNoise, observation, control1, control2, model )
    ...
    particlesPred = model.transition_fun(model, particles, xrand, control1);
    
    %% Evaluate importance weights
    likelihood = model.likelihood(model, column_vector_replicate(observation, cnt), particlesPred, control2, observNoise) + 1e-99;
    ...
    if (effectiveSetSize < resampleThreshold)
        ...
        weights   = column_vector_replicate(1 / cnt, cnt);
    else
    ...
    switch model.estimateType
        case 'mean'
            estimate = sum(row_vector_replicate(weights, model.stateDimension) .* particles, 2);
        case 'median'
            estimate = median(row_vector_replicate(weights, model.stateDimension) .* particles, 2);
        otherwise
