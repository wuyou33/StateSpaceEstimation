function [ noise ] = sampleGamma( noiseDataSet, count )
    % sampleGamma. Generate N (count) samples of a noise source specified by the noiseDataSet data structure (gamma stochastic processes).
    %
    %   [ noise ] = sampleGamma( noiseDataSet, count )
    %
    %   INPUT
