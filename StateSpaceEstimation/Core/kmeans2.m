function [ centres, options, post ] = kmeans2( centres, data, options )
    % kmeans2
    %   Algorithm to set the centres of a cluster model. The matrix data represents the data which is being clustered, with each row
    ...
    %       data    - matrix data represents the data which is being clustered, with each rowcorresponding to a vector;
    ...
    %
    %	options.absPrecision            is a measure of the absolute precision required for the value of CENTRES at the solution.  If the absolute difference between
    %                                   the values of CENTRES between two successive steps is less than options.absPrecision, then this condition is satisfied.
    %
    %	options.precision               is a measure of the precision required of the error function at the solution.  If the absolute difference between the error
    %                                   functions between two successive steps is less than options.precision, then this condition is satisfied. Both this and the previous
    ...
    %%
    logError = options.logError; % 1
    initializeFromData = options.initializeFromData; % 5
    precision = options.precision; % 3
    absPrecision = options.absPrecision; % 2
    iterationNumber = options.iterationNumber; % 14
    
    [ndata, dataDim] = size(data);
    [ncentres, dim]  = size(centres);
    
    if dataDim ~= dim; error('[ kmeans ] Data dimension does not match dimension of centres'); end
    if ncentres > ndata; error(' [ kmeans] More centres than data'); end
    ...
    for i = 1 : iterationNumber
        ...
        err = sum(minvals);
        if logError > 0; fprintf(1, 'Cycle %4d; Error %11.11f; \n', i, err); end
