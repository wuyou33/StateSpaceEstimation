function [ distanceMatrix ] = euclidean_distance( left, right )
    % euclidean_distance. Takes two matrices of vectors and calculates the squared Euclidean distance between them.
    ...
    %
    %   [ distanceMatrix ] = euclidean_distance( left, right )
    %
    %   INPUT
    %       left    M-by-N matrix;
    %       right   M-by-L matrix.
    ...
    if dimLeft2 ~= dimRight2
        error('[ euclidean_distance ] dimension mismatch');
    end
