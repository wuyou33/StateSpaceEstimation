function dy = EquationOfMotion(time, state, acceleration, angularVelocity, tEpoch, sampleTime, startTime, gravityModel, mass)
    ...
    sun_infl = sun_influence(tEpoch, state(1:3));
    moon_infl = moon_influence(tEpoch, state(1:3));
    
    quaternion = state(7:10);
    rotatedAccelearation = quaternion_rotation(quaternion, f, 2);
    
    gravAcc = gravityModel.eval_gravity_acceleration(time, state(1:3), mass);
    ...
    dy(7:10, 1) = -0.5 * quaternion_multiply(quaternion, [0; w]);
end
