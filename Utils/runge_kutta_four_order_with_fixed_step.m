function [ out ] = runge_kutta_four_order_with_fixed_step( ode_fun, y, x, span )
    % runge_kutta_four_order_with_fixed_step. Runge-Kutta ODE solver (used fixed step size).
    %
    ...
    %
    %   [ out ] = runge_kutta_four_order_with_fixed_step( ode_fun, y, x, span )
    %
    %   INPUT
    ...
    %
    k_1 = ode_fun(x, y);
    k_2 = ode_fun(x+0.5*span, y+0.5*span*k_1);
    k_3 = ode_fun((x+0.5*span), (y+0.5*span*k_2));
    k_4 = ode_fun((x+span), (y+k_3*span));
