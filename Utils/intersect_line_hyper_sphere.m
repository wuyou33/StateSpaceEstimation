function [ points ] = intersect_line_hyper_sphere(line, sphere, n)
    % intersect_line_hyper_sphere. Return intersection points between a line in hyper space and a hyper sphere.
    ...
    diff_centers = bsxfun(@minus, line(:, 1:n), sphere(:, 1:end-1));
    
    % equation coefficients
    a = sum(line(:, n:end) .* line(:, n:end), 2);    
    b = 2 * sum(bsxfun(@times, diff_centers, line(:, n+1:end)), 2);    
    c = sum(diff_centers.*diff_centers, 2) - sphere(:, end).*sphere(:, end);
