function angleMean = meanAngle(angleList, limits, dim)
    ...
    %
    %   Synopsis: ANGLEMEAN = angleMean = meanAngle(angleList, limits, dim)
    %   Input:    angleList : Input vector or matrix of angles (in degrees). Up
    %                         to two dimensions.
    %             limits    : Definition of angles: [-pi pi] or [0 2*pi] degrees.
    %             dim       : Dimension over which to operate.
    %   Output:   angleMean : Mean of ANGLES (in radians).
    %
